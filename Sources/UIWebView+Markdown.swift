...
extension UIWebView {
    ...
    func loadMarkdownString(markdown: String, atBaseURL baseURL: NSURL? = nil, withStyleSheet sheet: String? = nil) {
    ...
    /// Load Markdown HTML String
    func loadMarkdownString(markdown: String, atBaseURL baseURL: NSURL? = nil, withStylesheetFile filename: String, andSurroundedByHTML surround: String? = nil) {
        let URL = baseURL ?? NSURL(fileURLWithPath: NSBundle.mainBundle().bundlePath)
        let html = MarkdownParse.convertMarkdownString(markdown)
        let fullHTMLPage: String
